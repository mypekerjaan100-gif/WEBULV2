-- F6: bounded 6/12-month trend reusing the canonical F5 monthly aggregator.
...
begin
  ...
  if p_month_count not in (6,12) then
    raise exception 'month_count must be 6 or 12';
  end if;
  ...
  if p_unit_id is null then
    if not public.auth_can_manage_up3_operations(p_contract_id,p_up3_id) then
      raise exception 'Not authorized to read Financial Trend in this UP3' using errcode='42501';
    end if;
  elsif not public.auth_can_manage_pelayanan_teknik_scope(p_contract_id,p_up3_id,p_unit_id) then
    raise exception 'Not authorized to read Financial Trend in this scope' using errcode='42501';
  end if;
  ...
  for v_offset in 0..p_month_count-1 loop
    ...
    if exists(
      ...
      where version.contract_id=p_contract_id and version.up3_id=p_up3_id and version.status='ACTIVE'
        and v_period_month between version.period_start and version.period_end
    ) then
      ...
      v_has_data:=coalesce((v_summary->>'configured_target_count')::integer,0)>0
        ...
        or exists(
          select 1 from jsonb_array_elements(v_dashboard->'cells') cell
          where jsonb_array_length(coalesce(cell->'actual_indicator_ids','[]'::jsonb))>0
        )
        or exists(
          select 1 from public.variable_cost_konstruksi_monthly_amounts amount
          where amount.contract_id=p_contract_id and amount.up3_id=p_up3_id
            and amount.period_month=v_period_month
