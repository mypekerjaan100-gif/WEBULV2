-- D1 Financial Comparison Dashboard: aggregated revenue vs operational cost (Lembur) with server-side auth.
...
comment on function public.get_financial_comparison_dashboard(uuid,uuid,date,uuid) is 'D1 Financial Comparison: server-side aggregated revenue vs lembur cost, ADMIN_ULP denied, approved-only.';
