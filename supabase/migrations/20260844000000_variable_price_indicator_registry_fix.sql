-- F2.1: validate Harga Satuan against the canonical Variable priced set.
alter table public.variable_unit_prices
  drop constraint if exists variable_unit_prices_indicator_id_fkey;

drop policy if exists variable_unit_prices_select_scope on public.variable_unit_prices;
create policy variable_unit_prices_select_scope
on public.variable_unit_prices
for select
to authenticated
using (
...
begin
  ...
  if not public.auth_can_manage_pelayanan_teknik_scope(p_contract_id, p_up3_id, (select id from public.organization_units where parent_id=p_up3_id and type='ULP' limit 1)) then
  ...
  if v_effective is null then raise exception 'effective_from is required'; end if;
  ...
  where contract_id=p_contract_id and operational_up3_id=p_up3_id
    and indicator_id=p_indicator_id and effective_from=v_effective
  for update;
  if found then
    if v_existing.unit_price = p_unit_price then return v_existing; end if;
    update public.variable_unit_prices
    set unit_price=p_unit_price, updated_by=auth.uid()
    where id=v_existing.id
    returning * into v_result;
  else
    insert into public.variable_unit_prices
      (contract_id, operational_up3_id, indicator_id, unit_price, effective_from, created_by, updated_by)
    values
      (p_contract_id, p_up3_id, p_indicator_id, p_unit_price, v_effective, auth.uid(), auth.uid())
    returning * into v_result;
  end if;
  return v_result;
end;
...
declare
  v_val jsonb;
  v_res public.variable_unit_prices%rowtype;
begin
  if jsonb_typeof(p_values) is distinct from 'array' or jsonb_array_length(p_values)=0 then
    raise exception 'values must be non-empty array';
  end if;
  ...
  for v_val in select value from jsonb_array_elements(p_values) loop
    select * into v_res
    from public.set_variable_unit_price(
      p_contract_id,
      p_up3_id,
      (v_val->>'indicator_id')::uuid,
      p_effective_from,
      (v_val->>'unit_price')::numeric
    );
    return next v_res;
  end loop;
  return;
...
set search_path to 'public','pg_temp'
as $function$
  select distinct on (vup.indicator_id)
    vup.indicator_id, vup.unit_price, vup.effective_from
  from public.variable_unit_prices vup
  where vup.contract_id=p_contract_id
    and vup.operational_up3_id=p_up3_id
    and (p_as_of is null or vup.effective_from <= p_as_of)
    and public.is_priced_variable_indicator(p_contract_id, p_up3_id, vup.indicator_id)
  ...
  order by vup.indicator_id, vup.effective_from desc;
$function$;
