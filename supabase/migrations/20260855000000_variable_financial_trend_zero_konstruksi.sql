-- Treat an explicitly stored Rp0 Konstruksi actual as financial data.
  ...
  for v_offset in 0..p_month_count-1 loop
    ...
    if exists(select 1 from public.sla_versions version where version.contract_id=p_contract_id and version.up3_id=p_up3_id and version.status='ACTIVE' and v_period_month between version.period_start and version.period_end) then
      ...
      v_has_data:=coalesce((v_summary->>'configured_target_count')::integer,0)>0
        ...
        or exists(select 1 from public.variable_cost_konstruksi_monthly_amounts amount where amount.contract_id=p_contract_id and amount.up3_id=p_up3_id and amount.period_month=v_period_month and (p_unit_id is null or amount.unit_id=p_unit_id) and public.auth_can_manage_pelayanan_teknik_scope(p_contract_id,p_up3_id,amount.unit_id) and exists(select 1 from public.organization_units unit where unit.id=amount.unit_id and unit.type='ULP' and unit.parent_id=p_up3_id and unit.own_status='Aktif'));
      v_months:=v_months||jsonb_build_array(jsonb_build_object(
        'period_month',v_period_month,
        'target_amount',coalesce((v_summary->>'target_amount')::numeric,0),
        ...
        'required_target_count',v_required_count,
        'missing_target_count',coalesce((v_summary->>'missing_target_count')::integer,0),
        'target_complete',v_required_count>0 and coalesce((v_summary->>'missing_target_count')::integer,0)=0,
        'missing_price_count',coalesce((v_summary->>'missing_price_count')::integer,0),
        'has_data',v_has_data
      ));
    else
      v_months:=v_months||jsonb_build_array(jsonb_build_object(
        'period_month',v_period_month,'target_amount',0,'actual_amount',0,
        'difference_amount',null,'achievement_percent',null,
        'configured_target_count',0,'required_target_count',0,'missing_target_count',0,
        'target_complete',false,'missing_price_count',0,'has_data',false
      ));
    end if;
  end loop;
