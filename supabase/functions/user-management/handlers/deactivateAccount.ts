...
export async function handleDeactivateAccount(
  ...
  if (profile.status === "DISABLED") return { status: 200, body: { status: "ALREADY_DISABLED" } };
  const { error } = await adminClient.rpc("admin_deactivate_user", {
    p_target_user_id: targetUserId,
    p_actor_id: actorUserId,
  });
  if (error) return { status: 500, body: { error: "deactivate_failed", message: error.message } };
